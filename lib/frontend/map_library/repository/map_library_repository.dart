...
class MapLibraryRepository {
  ...
  Future<MapLibraryData> loadAll() async {
    ...
    final List<MapLibraryItem> maps = mapsRaw
        .whereType<Map>()
        .map((Map raw) => MapLibraryItem.fromJson(Map<String, dynamic>.from(raw)))
        .where((MapLibraryItem item) => item.key.isNotEmpty && item.name.isNotEmpty)
        .toList(growable: false)
      ..sort((MapLibraryItem a, MapLibraryItem b) => a.name.compareTo(b.name));

    final Map<String, MapMonsterInfo> monsterById = <String, MapMonsterInfo>{
      for (final MapMonsterInfo monster in monstersRaw
          .whereType<Map>()
          .map(
            (Map raw) => MapMonsterInfo.fromJson(Map<String, dynamic>.from(raw)),
          )
